...
import net.etri.rest.api.filter.XmlFIleFilter;
...
public class XmlManager {
	...
	public void TrafficSignalToFile(TrafficSignals trafficSignalSystem, File file) throws JAXBException, IOException{
		...
		// 20211119 장길수 : 신호 버튼에 체크되어 있지 않았을때 발생하는 오류 수정
	...
	// 20211118 장길수 : 버스 정류소 XML 반환
	public void BusStopToFile(BusStops busstops, File file) throws JAXBException, IOException {
		// TODO Auto-generated method stub
		if (busstops != null) {
			JAXBContext ctx = JAXBContext.newInstance(BusStops.class);		
			Marshaller m = ctx.createMarshaller();
	        m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
	        
	        m.marshal(busstops, file);
		}
	}
	...
	public void ScenearioXmlResponse(Scenarios sc, HttpServletResponse response, String path) throws JAXBException, IOException{
		...
		BusStopToFile(sc.getBusstops(), new File(path + File.separator + currentTime +  File.separator + "busstops.xml"));
	...
	public void PartitionXmlResponse(PartitionScenarios partitions, HttpServletResponse response, String path) throws JAXBException, IOException{
		...
		if( partitions.getPartition().size() == 1 ){
			...
			// 20211118 장길수 추가 : 버스 정류소 목록
			BusStopToFile(partitions.getPartition().get(0).getBusstops(), new File(path + File.separator + currentTime +  File.separator + "busstops.xml"));
		}else{
			for(int i = 0; i < partitions.getPartition().size(); i++ ){
				...
				// 20211118 장길수 추가 : 버스 정류소 목록
				BusStopToFile(partitions.getPartition().get(i).getBusstops(), new File(path + File.separator + currentTime +  File.separator + "sub" + i + ".busstops.xml"));
			}
		}
