...
import net.etri.rest.api.mapper.XmlMapper;
...
//	}
		...
		try {
			...
			if( list != null && !list.isEmpty()){
				...
				while( it.hasNext()){
					...
					//////////////////////////////////////////////////////////////////////////
					HashMap<String, Object> map = new HashMap<String, Object>();
					map.put("signal_id", signal.getSignal_id());
					map.put("version", signal.getVersion());
					map.put("time_plan", 0);
					List<SignalTime> signalTimeList = session.getMapper(XmlMapper.class).selectSignalTime(map);
					signal.setSignalTime(signalTimeList);
					//////////////////////////////////////////////////////////////////////////
	...
	*/
	...
	public List<BusStop> selectBusStopList(List<Edge> edge) {
		try {
			return session.getMapper(XmlMapper.class).selectBusStopList(edge);
		} catch (Exception e) {
			log.error("selectBusStopList", e);
		}
	...
	public List<Node> CoordinateNodeByDong(RequestVO vo) {
		try {
			return session.getMapper(XmlMapper.class).CoordinateNodeByDong(vo);
		} catch (Exception e) {
			log.error("CoordinateNode", e);
		}
		return null;
